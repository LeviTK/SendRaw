...
from qt.core import (
    ...
    QListWidget, QPushButton, QGroupBox, QSpinBox, QCheckBox
)
...
prefs.defaults['verify_md5'] = True
...
class ConfigWidget(QWidget):
    ...
    def init_ui(self):
        ...
        transfer_layout.addWidget(QLabel('可用变量: {title}, {author}, {series}, {series_index}'))
    ...
    def save_settings(self):
        ...
        prefs['filename_template'] = self.template_edit.text().strip() or '{author} - {title}'
    ...
    def add_format(self):
        ...
        fmt, ok = QInputDialog.getText(
            self, '添加格式', 
            '输入格式名称 (例如: EPUB, PDF, MOBI):'
        )
        if ok and fmt:
            fmt = fmt.upper().strip()
            existing = [self.format_list.item(i).text() for i in range(self.format_list.count())]
